...
export const executeTerminalCommand: RequestHandler = async (req, res) => {
  try {
    const { command, workingDirectory = process.cwd() } = terminalCommandSchema.parse(req.body);
...
async function executeJavaScript(code: string, input?: string): Promise<{ output: string; error?: string; executionTime: number }> {
...
async function executePython(code: string, input?: string): Promise<{ output: string; error?: string; executionTime: number }> {
  return new Promise((resolve) => {
...
async function executeTypeScript(code: string, input?: string): Promise<{ output: string; error?: string; executionTime: number }> {
  try {
    ...
    return new Promise((resolve) => {
      // Compile TypeScript
      const tsc = spawn('npx', ['tsc', tsFile, '--outDir', tempDir, '--target', 'ES2020'], {
        stdio: ['pipe', 'pipe', 'pipe']
      });
...
async function executeShellCommand(command: string, workingDirectory: string): Promise<{ output: string; error?: string }> {
  return new Promise((resolve) => {
    // Security: Only allow safe commands
    const safeCommands = ['ls', 'pwd', 'echo', 'cat', 'grep', 'find', 'wc', 'head', 'tail', 'npm', 'node', 'python', 'pip'];
...
async function generateJoseyResponse(message: string, context?: any): Promise<{ content: string; type: string }> {
