...
export interface NetlifyState {
  ...
  updateEnvironmentVariables: (siteId: string, variables: Record<string, string>) => Promise<boolean>;
  
  // Webhooks
  createWebhook: (siteId: string, url: string, events: string[]) => Promise<boolean>;
...
export const useNetlifyStore = create<NetlifyState>((set, get) => ({
  ...
  connectBuildLogs: (buildId: string) => {
    ...
    const interval = setInterval(() => {
      if (index < messages.length) {
        get().addBuildLog(buildId, `[${new Date().toLocaleTimeString()}] ${messages[index]}`);
        index++;
      } else {
  ...
  updateEnvironmentVariables: async (siteId: string, variables: Record<string, string>) => {
    set({ loading: true, error: null });
    try {
      await new Promise(resolve => setTimeout(resolve, 800));
